...
import { ArrowLeft, Plus, Edit, Trash2, Save, X, Settings, Building2, Users, Check, Copy } from 'lucide-react';
import { AnalysisAxis, Client } from '@/lib/types';
import AdminNavigation from '@/app/components/AdminNavigation';

export default function ClientAxesPage() {
  const params = useParams();
  const router = useRouter();
  const clientId = params.clientId as string;
  
  const [client, setClient] = useState<Client | null>(null);
  const [defaultAxes, setDefaultAxes] = useState<AnalysisAxis[]>([]);
  const [clientAxes, setClientAxes] = useState<AnalysisAxis[]>([]);
  ...
  const fetchData = async () => {
    try {
      ...
      // Récupérer les axes par défaut
      const defaultAxesResponse = await fetch('/api/analysis-axes');
      const defaultAxesData = await defaultAxesResponse.json();
      setDefaultAxes(defaultAxesData);

      // Récupérer les axes spécifiques au client
      const clientAxesResponse = await fetch(`/api/client-analysis-axes?clientId=${clientId}`);
      const clientAxesData = await clientAxesResponse.json();
      setClientAxes(clientAxesData);
  ...
  const handleSaveAxis = async (axisData: Partial<AnalysisAxis>) => {
    try {
      const url = '/api/client-analysis-axes';
  ...
  const handleDeleteAxis = async (id: string) => {
    if (confirm('Êtes-vous sûr de vouloir supprimer cet axe spécifique au client ?')) {
      try {
        const response = await fetch(`/api/client-analysis-axes?id=${id}&clientId=${clientId}`, {
          method: 'DELETE'
        });
  ...
  const handleCopyFromDefault = async (defaultAxis: AnalysisAxis) => {
    try {
      const response = await fetch('/api/client-analysis-axes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...defaultAxis,
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        alert(`Erreur lors de la copie de l'axe: ${errorData.error}`);
      }
    } catch (error) {
      console.error('Erreur lors de la copie de l\'axe:', error);
      alert('Erreur lors de la copie de l\'axe.');
    }
  };
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-4xl font-bold text-gray-800">
              Axes d'Analyse - {client.name}
            </h1>
            <p className="text-gray-600 mt-2">
              Configurez les axes d'analyse spécifiques à ce client
            </p>
          </div>
        </div>

        {/* Axes spécifiques au client */}
        <div className="bg-white shadow-lg rounded-lg p-6 mb-8">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-2xl font-semibold text-gray-700">Axes Spécifiques au Client</h2>
              <p className="text-gray-600 mt-2">
                Ces axes remplaceront les axes par défaut pour ce client uniquement.
              </p>
            </div>
            <button
              onClick={() => {
            ...
            >
              ...
              Ajouter un Axe Spécifique
            </button>
          </div>

          {clientAxes.length === 0 ? (
            <div className="text-center py-10">
              <p className="text-gray-500 text-lg mb-4">Aucun axe spécifique configuré pour ce client.</p>
              <p className="text-gray-400 text-sm">Les axes par défaut seront utilisés.</p>
            </div>
          ) : (
        ...
        <div className="bg-white shadow-lg rounded-lg p-6">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-2xl font-semibold text-gray-700">Axes par Défaut</h2>
              <p className="text-gray-600 mt-2">
                Cliquez sur "Copier" pour créer une version spécifique à ce client.
              </p>
            </div>
          </div>

          {defaultAxes.length === 0 ? (
            <p className="text-gray-500">Aucun axe par défaut configuré.</p>
          ) : (
            <div className="space-y-4">
              {defaultAxes
                .sort((a, b) => a.order - b.order)
                .map((axis) => {
                  const isAlreadyCopied = clientAxes.some(ca => ca.name === axis.name);
                  ...
                  return (
                    <div key={axis.id} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          <div className="flex items-center gap-2">
                            <span className="w-8 h-8 bg-gray-100 text-gray-600 rounded-full flex items-center justify-center text-sm font-bold">
                              {axis.order}
                            </span>
                            <div>
                              <h3 className="text-lg font-semibold text-gray-800">{axis.name}</h3>
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => handleCopyFromDefault(axis)}
                            ...
                            title={isAlreadyCopied ? 'Déjà copié' : 'Copier vers les axes spécifiques'}
                          >
                            <Copy className="w-4 h-4" />
                            {isAlreadyCopied ? 'Copié' : 'Copier'}
                          </button>
                        </div>
                      </div>
                    </div>
